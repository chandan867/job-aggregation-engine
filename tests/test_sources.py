...
def test_cli_default_path_discovers_and_fetches_ats_sources(monkeypatch, tmp_path, capsys):
    from job_aggregation_engine import cli

    calls = []
    monkeypatch.setattr(cli, "submodule_preflight", lambda **kwargs: [])
    monkeypatch.setattr(cli, "jobspy_search", lambda *args: calls.append("jobspy") or [])
    monkeypatch.setattr(cli, "discover_freehire_boards", lambda *args, **kwargs: calls.append("freehire") or [("greenhouse", "acme", "Acme")])
    monkeypatch.setattr(cli, "ats_scrapers_search", lambda *args, **kwargs: calls.append("ats") or ([], 2))

    cli.main(["--title", "Engineer", "--location", "London", "--output", str(tmp_path / "jobs.jsonl")])

    assert calls == ["jobspy", "freehire", "ats"]
    health = json.loads(capsys.readouterr().err.removeprefix("source_health=").strip())
